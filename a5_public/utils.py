...
def pad_sents_char(sents, char_pad_token):
    ...
    for s in sents:
        ...
        for w in s:
            len_w = len(w)
            if len_w >= max_word_length:
                tmp_s.append(w[:max_sentence_length])
            else:
                tmp_s.append(w+[char_pad_token]*(max_word_length-len_w))
        len_s = len(tmp_s)
        if len_s < max_sentence_length:
            tmp_s += [[char_pad_token for _ in range(max_word_length)]for _ in range(max_sentence_length-len_s)]
