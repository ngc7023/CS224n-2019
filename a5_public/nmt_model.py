...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True) # change to (b, src_len, h*2) src_len:max_len
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        alpha_t = F.softmax(e_t,dim =1) # attention_distribution
        alpha_t = torch.unsqueeze(alpha_t, dim = 1)
        a_t = torch.bmm(alpha_t, enc_hiddens)
        a_t = torch.squeeze(a_t, dim=1) # attention output (b, 2h)
